import { getSession, login, signup, logout, isInternalNetwork, pullUsers, isAdmin, listUsers, pendingCount, setUserStatus } from "./auth.js?v=42";
import {
    ...
    <div class="app ${printMode ? "print-mode" : ""}">
      ...
      <aside class="side">
        <div class="side-block go">
          ...
          <a href="./cam-lab.html?v=20">${ht("가공 프로그램")}</a>
        </div>
...
function isCamNc(name) {
  return /\.(nc|nci|cnc|tap|mc9)$/i.test(name || "");
}
...
async function ingestCamFile(file, date = todayISO()) {
  ...
  const text = await file.slice(0, 2_000_000).text().catch(() => "");
